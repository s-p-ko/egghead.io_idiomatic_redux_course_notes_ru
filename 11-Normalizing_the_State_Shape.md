...
В настоящее время мы представляем `todos` в дереве состояний как массив `todo` объектов. Но в реальном приложении у нас, вероятно, будет более одного массива, и `todos` с одинаковыми `id` в разных массивах могут не синхронизироваться.
...
Начнем с переименования редюсера в `byId`. Теперь, вместо того, чтобы добавлять новый элемент в конец или отображать каждый элемент, мы изменим значение в таблице поиска.
...
Теперь, когда мы сохраняем сами задачи в `byId` карте (map), у нас будет состояние этого редюсера как массива идентификаторов (`id`s).
...
---
_Примечание_: вы можете использовать объединенные (combined) редюсеры сколько угодно раз. Вам не обязательно использовать его только на редюсере верхнего уровня. Фактически, очень часто по мере роста вашего приложения вы будете использовать `combineReducers` в нескольких местах.
...
Поскольку массив todos мы больше не используем, напишем селектор `getAllTodos`, чтобы создать массив для нас.
...
`getAllTodos` не будем экспортировать, потому что он будет использоваться только в текущем файле.
